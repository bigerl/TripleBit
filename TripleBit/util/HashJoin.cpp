...
//	/*
	if (arg->length2 > 100 && arg->length1 > 100) {
		...
		HashJoin::BuildHashIndex(arg->buffer1->getBuffer() + arg->startPos1 * arg->buffer1->IDCount, arg->joinKey1,
		                         hashKey, hashBucket1,
		                         prefixSum1, arg->length1, arg->buffer1->IDCount);
		HashJoin::BuildHashIndex(arg->buffer2->getBuffer() + arg->startPos2 * arg->buffer2->IDCount, arg->joinKey2,
		                         hashKey, hashBucket2,
			...
			if (hashBucket1[i] != 0 && hashBucket2[i] != 0) {
				...
				if (arg->flag2 != NULL)
					HashJoin::SortMergeJoin(
							arg->buffer1->getBuffer() + (arg->startPos1 + start1) * arg->buffer1->IDCount,
