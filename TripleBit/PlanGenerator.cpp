...
{
	for (vector<TripleBitQueryGraph::Filter>::const_iterator iter = query.filters.begin(), limit = query.filters.end();
	     iter != limit; ++iter)
		if ((*iter).id == val)
			return false;
	for (vector<TripleBitQueryGraph::TripleNode>::const_iterator iter = query.tripleNodes.begin(), limit = query.tripleNodes.end();
	     iter != limit; ++iter) {
	...
	for (vector<TripleBitQueryGraph::SubQuery>::const_iterator iter = query.optional.begin(), limit = query.optional.end();
	     iter != limit; ++iter)
		if (!isUnused(*iter, node, val))
			return false;
	for (vector<vector<TripleBitQueryGraph::SubQuery> >::const_iterator iter = query.unions.begin(), limit = query.unions.end();
	     iter != limit; ++iter)
		for (vector<TripleBitQueryGraph::SubQuery>::const_iterator iter2 = (*iter).begin(), limit2 = (*iter).end();
		     iter2 != limit2; ++iter2)
...
static bool isUnused(const TripleBitQueryGraph &graph, const TripleBitQueryGraph::TripleNode &node, unsigned val) {
	// check if a variable is unused outside its primary pattern.
	for (TripleBitQueryGraph::projection_iterator begin = graph.projectionBegin(), limit = graph.projectionEnd();
	     begin != limit; begin++) {
		if (*begin == val)
			return false;
	}
...
Status PlanGenerator::generatePlan(TripleBitQueryGraph &_graph) {
	...
	for (vector<TripleBitQueryGraph::TripleNode>::iterator iter = query->tripleNodes.begin(), limit = query->tripleNodes.end();
	     iter != limit; iter++) {
...
//	CThreadPool::getInstance().Wait();
	...
	for (; joinVariableIter != query->joinVariableNodes.end(); joinVariableIter++) {
		...
		if (joinType == TripleBitQueryGraph::JoinVariableNode::SP ||
		    joinType == TripleBitQueryGraph::JoinVariableNode::OP) {
...
//		cout<<"join var:" << joinVariableIter->value << "  select:" << selectivityMap[joinVariableIter->value] << endl;
	...
	Status state_ = bfsTraverseVariableNode();
	if (state_ != OK)
		return ERROR;
...
Status PlanGenerator::generateScanOperator(TripleBitQueryGraph::TripleNode &node,
...
TripleBitQueryGraph::JoinVariableNode::JoinType
PlanGenerator::getJoinType(TripleBitQueryGraph::JoinVariableNode &node) {
...
Status PlanGenerator::generateSelectivity(TripleBitQueryGraph::JoinVariableNode &node,
                                          map<TripleBitQueryGraph::JoinVariableNodeID, int> &selectivityMap) {
...
Status PlanGenerator::getAdjVariableByID(TripleBitQueryGraph::JoinVariableNodeID id,
