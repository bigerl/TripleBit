...
extern bool
isUnused(const TripleBitQueryGraph::SubQuery &query, const TripleBitQueryGraph::TripleNode &node, unsigned val);

//---------------------------------------------------------------------------
static bool binds(const SPARQLParser::PatternGroup &group, ID id)
// Check if a variable is bound in a pattern group
{
	for (std::vector<SPARQLParser::Pattern>::const_iterator iter = group.patterns.begin(), limit = group.patterns.end();
	     iter != limit; ++iter)
		if ((((*iter).subject.type == SPARQLParser::Element::Variable) && ((*iter).subject.id == id)) ||
			...
			return true;
	for (std::vector<SPARQLParser::PatternGroup>::const_iterator iter = group.optional.begin(), limit = group.optional.end();
	     iter != limit; ++iter)
		if (binds(*iter, id))
			return true;
	for (std::vector<std::vector<SPARQLParser::PatternGroup> >::const_iterator iter = group.unions.begin(), limit = group.unions.end();
	     iter != limit; ++iter)
		for (std::vector<SPARQLParser::PatternGroup>::const_iterator iter2 = (*iter).begin(), limit2 = (*iter).end();
		     iter2 != limit2; ++iter2)
			if (binds(*iter2, id))
				return true;
	return false;
...
//---------------------------------------------------------------------------
static bool encodeFilter(IRepository &repo, const SPARQLParser::PatternGroup &group, const SPARQLParser::Filter &input,
...
// Encode an element for the query graph
{
	...
	for (std::vector<SPARQLParser::Element>::const_iterator iter = input.values.begin(), limit = input.values.end();
	     iter != limit; ++iter) {
		unsigned id;
...
//--------------------------------------------------------------------------
bool static encodeTripleNode(IRepository &repo, const SPARQLParser::Pattern &triplePattern,
...
static bool encodeJoinVariableEdges(TripleBitQueryGraph::SubQuery &output) {
 ...
 // should be sth like: for (i = 0; long(i) < long(long(size) - 1); i++) {
 // this is dangerous and most probably wrong
	// for( i= 0; i < size - 1; i++)
	for (i = 0; long(i) < long(long(size) - 1); i++)
	{
...
//---------------------------------------------------------------------------
static bool
transformSubquery(IRepository &repo, const SPARQLParser::PatternGroup &group, TripleBitQueryGraph::SubQuery &output)
// Transform a subquery, encoding PatternGroup, fill the subquery,build the JoinVariable Nodes and their edges with triple nodes.
{
	...
	for (vector<SPARQLParser::Filter>::const_iterator iter = group.filters.begin(), limit = group.filters.end();
	     iter != limit; iter++) {
...
// Perform the transformation
{
	...
	for (SPARQLParser::projection_iterator iter = input.projectionBegin(), limit = input.projectionEnd();
	     iter != limit; ++iter) {
