...
void TripleBitQuery::displayAllTriples() {
	...
	cout << "The total tripleCount is : " << tripleCount << endl << "Do you really want to display all (Y / N) ?"
	     << endl;
	char displayFlag;
	//cin>>displayFlag;
	displayFlag = 'y';
		...
		for (i = 0; i != _query->tripleNodes.size(); i++) {
			...
			if (tempBitV) {
				...
				if (EntityIDList.count(_query->tripleNodes[i].tripleNodeID) == 0 ||
				    (EntityIDList[_query->tripleNodes[i].tripleNodeID]->getIDCount() == 1))
					continue;
...
bool TripleBitQuery::getResult_with_flags(ID key, std::vector<EntityIDBuffer *> &bufferlist, size_t buflist_index,
...
void TripleBitQuery::getResult_join_with_flags(ID key, std::vector<EntityIDBuffer *> &bufferlist, size_t buflist_index,
                                               vector<unsigned> flags) {
	if (buflist_index == bufferlist.size())
		return;
...
EntityType
TripleBitQuery::getKnowBufferType(TripleBitQueryGraph::TripleNode *node1, TripleBitQueryGraph::TripleNode *node2) {
...
Status TripleBitQuery::findEntityIDByTriple(TripleBitQueryGraph::TripleNode *triple, EntityIDBuffer *buffer, ID minID,
                                            ID maxID, unsigned maxNum) {
...
//	 gettimeofday(&start,NULL);
	...
	switch (triple->scanOperation) {
		case TripleBitQueryGraph::TripleNode::FINDSBYPO:
			status = entityFinder->findSubjectIDByPredicateAndObject(triple->predicate, triple->object, buffer, minID,
			                                                         maxID);
			break;
		case TripleBitQueryGraph::TripleNode::FINDOBYSP:
			status = entityFinder->findObjectIDByPredicateAndSubject(triple->predicate, triple->subject, buffer, minID,
			                                                         maxID);
			break;
		case TripleBitQueryGraph::TripleNode::FINDSOBYP:
...
Status TripleBitQuery::findEntityIDByKnowBuffer(TripleBitQueryGraph::TripleNode *triple, EntityIDBuffer *buffer,
...
Status TripleBitQuery::acyclicJoin() {
	...
	if (projectionSize == 1) {
		...
		while (true) {
			if (EntityIDList.count(_query->tripleNodes[i].tripleNodeID) == 0 ||
			    (EntityIDList[_query->tripleNodes[i].tripleNodeID]->getIDCount() == 1)
			    || (find(bufferlist.begin(), bufferlist.end(), EntityIDList[_query->tripleNodes[i].tripleNodeID]) !=
			        bufferlist.end())) {
				i++;
				if (i == tnodesize) {
					cycles++;
...
Status TripleBitQuery::cyclicJoin() {
	...
	if (_query->joinVariables.begin() == _query->joinVariables.end())
		return ERROR;
	getVariableNodeByID(node, *joinNodeIter);
	nodePatternIter = node->appear_tpnodes.begin();
	buffer = BufferManager::getInstance()->getNewBuffer();
...
int
TripleBitQuery::getVariablePos(TripleBitQueryGraph::JoinVariableNodeID id, TripleBitQueryGraph::TripleNode *triple) {
	int pos;
...
Status
TripleBitQuery::getTripleNodeByID(TripleBitQueryGraph::TripleNode *&triple, TripleBitQueryGraph::TripleNodeID nodeID) {
...
EntityType TripleBitQuery::getDimInTriple(
		vector<pair<TripleBitQueryGraph::TripleNodeID, TripleBitQueryGraph::JoinVariableNode::DimType> > &tpnodes,
...
Status TripleBitQuery::findEntitiesAndJoinFirstTime(
		vector<pair<TripleBitQueryGraph::TripleNodeID, TripleBitQueryGraph::JoinVariableNode::DimType> > &tpnodes,
...
Status TripleBitQuery::modifyEntitiesAndJoin(
		vector<pair<TripleBitQueryGraph::TripleNodeID, TripleBitQueryGraph::JoinVariableNode::DimType> > &tpnodes,
...
Status TripleBitQuery::getVariableNodeByID(TripleBitQueryGraph::JoinVariableNode *&node,
